...
def parseTrackTLV(tlvData, tlvLength, outputDict):
    """
    Parse and print the TRACKED OBJECTS TLV data.
    """
    ...
    print(f"\n[INFO] Parsing eTrack TLV:")
    print(f"  Total TLV length: {tlvLength} bytes")
    print(f"  Structure size per target: {targetSize} bytes")
    print(f"  Number of detected targets: {numDetectedTargets}\n")

    # Print header
    print(f"{'ID':<4} {'X':>8} {'Y':>8} {'Z':>8} {'Vx':>8} {'Vy':>8} {'Vz':>8} {'Ax':>8} {'Ay':>8} {'Az':>8} {'G':>8} {'Conf':>8}")
    print("-" * 100)
    ...
    for i in range(numDetectedTargets):
        try:
            ...
            print(f"[ERROR] Target TLV parsing failed: {e}")
            outputDict['numDetectedTracks'], outputDict['trackData'] = 0, targets
            return

        # Fill in array
        targets[i, 0]  = targetData[0]   # ID
        targets[i, 1]  = targetData[1]   # X
        targets[i, 2]  = targetData[2]   # Y
        targets[i, 3]  = targetData[3]   # Z
        targets[i, 4]  = targetData[4]   # Vx
        targets[i, 5]  = targetData[5]   # Vy
        targets[i, 6]  = targetData[6]   # Vz
        targets[i, 7]  = targetData[7]   # Ax
        targets[i, 8]  = targetData[8]   # Ay
        targets[i, 9]  = targetData[9]   # Az
        targets[i,10]  = targetData[26]  # G
        targets[i,11]  = targetData[27]  # Confidence

        # Print one target per line
        print(f"{int(targets[i,0]):<4} {targets[i,1]:8.3f} {targets[i,2]:8.3f} {targets[i,3]:8.3f} "
              f"{targets[i,4]:8.3f} {targets[i,5]:8.3f} {targets[i,6]:8.3f} {targets[i,7]:8.3f} "
              f"{targets[i,8]:8.3f} {targets[i,9]:8.3f} {targets[i,10]:8.3f} {targets[i,11]:8.3f}")

        # Move to next target block
        tlvData = tlvData[targetSize:]

    outputDict['numDetectedTracks'] = numDetectedTargets
