...
class Core:
    ...
    def parseCfg(self, fname):
        ...
        for line in self.cfg:
            ...
            if len(args) > 0:
                ...
                elif args[0] in ["sensorPosition", "sensorPosition1", "sensorPosition2"]:
                    # Determine which radar index this is (0, 1, or 2)
                    if args[0] == "sensorPosition":
                        radar_index = 0
                    elif args[0] == "sensorPosition1":
                        radar_index = 1
                    elif args[0] == "sensorPosition2":
                        radar_index = 2
                    ...
                    if DEVICE_DEMO_DICT[self.device]["isxWRx843"] and len(args) < 4:
                        ...
                        with suppress(AttributeError):
                            self.demoClassDict[self.demo].parseSensorPosition(
                                args, DEVICE_DEMO_DICT[self.device]["isxWRx843"], radar_index
                            )
    ...
    def updateGraph(self, outputDict):
        print("Updating Graph fin connect function")
